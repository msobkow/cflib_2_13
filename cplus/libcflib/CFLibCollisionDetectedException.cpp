...
namespace cflib {
	...
	const std::string CFLibCollisionDetectedException::CLASS_NAME( "CFLibCollisionDetectedException" );
	const std::string CFLibCollisionDetectedException::CAUSE_NAME( "CollisionDetected" );
	...
	: cflib::CFLibRuntimeException(
			...
			S_Empty,					// argArgMinValueString
			S_Empty,					// argArgMaxValueString
			std::string( S_COLLISION_DETECTED + (( argKey != NULL ) ? argKey->toString() : CFLib::S_EMPTY )) )
	{
	}
}
