...
namespace cflib {
	...
	const std::string CFLibDependentsDetectedException::CLASS_NAME( "CFLibDependentsDetectedException" );
	const std::string CFLibDependentsDetectedException::CAUSE_NAME( "DependentsDetected" );
	...
	: cflib::CFLibIntegrityException(
			...
			S_Empty,					// argArgMinValueString
			S_Empty,					// argArgMaxValueString
			std::string( relationType + S_RELATION + relationName + S_DEPENDENTS_DETECTED_FOR_KEY + (( argKey != NULL ) ? argKey->toString() : CFLib::S_EMPTY ) ) )
	{
	}
}
