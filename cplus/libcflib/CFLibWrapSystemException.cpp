...
namespace cflib {
	...
	const std::string CFLibWrapSystemException::CLASS_NAME( "CFLibWrapSystemException" );
	const std::string CFLibWrapSystemException::CAUSE_NAME( "SystemException" );

	CFLibWrapSystemException::~CFLibWrapSystemException() {
	}

	CFLibWrapSystemException::CFLibWrapSystemException(
		const CFLibWrapSystemException& src )
	...
	CFLibWrapSystemException::CFLibWrapSystemException(
		const std::string& throwingClassName,
		const std::string& methName,
		const std::string& caughtExceptionName,
		const std::string& caughtWhat )
	: cflib::CFLibRuntimeException(
			CAUSE_NAME,					// Cause
			CLASS_NAME,					// ExceptionName
			throwingClassName,			// argClassFieldName
			...
			S_Empty,					// argArgMinValueString
			S_Empty,					// argArgMaxValueString
			"Caught system exception " + caughtExceptionName + " - " + caughtWhat )		// argMessageText
	{
	}
}
